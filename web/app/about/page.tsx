...
export default function AboutPage() {
  return <main><SiteHeader /><PageHero eyebrow="ABOUT / 關於" title="Build. Learn. Share." description="NTUMaker 致力於推廣創客文化。這裡不要求你一開始就會，而是希望每個人都能找到一起做東西、交換方法和完成作品的夥伴。" />
    ...
    ].map((item,index)=>{const Icon=item.icon;return <article key={item.title} className={`min-h-80 rounded-[2rem] border border-border p-8 ${index===1?"bg-secondary text-on-secondary":"bg-surface"}`}><Icon size={32}/><h2 className="mt-20 text-4xl font-black">{item.title}</h2><p className={`mt-4 leading-7 ${index===1?"text-white/75":"text-muted-foreground"}`}>{item.text}</p></article>;})}</div></section><SiteFooter /></main>;
}
