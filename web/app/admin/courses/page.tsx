      ...
      <div className="mx-auto max-w-[1280px]">
        ...
        {loading ? <div className="flex min-h-64 items-center justify-center gap-3 text-muted-foreground"><LoaderCircle className="animate-spin" />正在讀取課程…</div> : seriesList.length === 0 ? <div className="mt-5 border border-border bg-surface p-10 text-center text-muted-foreground">尚未建立課程路線，請先新增星期二或星期五課程。</div> : (
          <div className="mt-3 grid gap-6 xl:grid-cols-[minmax(0,1.1fr)_minmax(360px,.9fr)]">
            <div className="grid content-start gap-5">{seriesList.map((series) => <section key={series.id} className="border border-border bg-surface p-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className={`inline-flex border px-2 py-1 text-xs font-bold ${series.track === "tuesday" ? "border-primary text-primary" : "border-secondary text-secondary"}`}>{trackLabel(series.track)} · {series.semester}</p><h2 className="mt-3 text-xl font-black">{series.title}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{series.description}</p></div><div className="flex gap-2"><IconButton label={`編輯 ${series.title}`} onClick={() => openEditSeries(series)}><Edit3 size={16} /></IconButton><button onClick={() => openNewSession(series)} className="inline-flex min-h-11 items-center gap-2 border border-border px-3 text-sm font-bold"><CalendarPlus size={16} />新增課堂</button></div></div><div className="mt-5 grid gap-2">{series.sessions.length === 0 ? <p className="border-t border-border py-5 text-sm text-muted-foreground">這條路線還沒有課堂。</p> : series.sessions.map((session) => <button key={session.id} onClick={() => setSelectedSessionId(session.id)} className={`grid min-h-16 w-full grid-cols-[5rem_1fr_auto] items-center gap-3 border p-3 text-left transition-colors ${selectedSessionId === session.id ? "border-secondary bg-secondary/10" : "border-border hover:bg-background"}`}><span className="font-mono text-xs text-muted-foreground">{session.week_label}</span><span><span className="block font-bold">{session.title}</span><span className="mt-1 block text-xs text-muted-foreground">{session.resources.length} 項內容</span></span><span className="text-xs font-bold text-secondary">管理</span></button>)}</div></section>)}</div>

            <aside className="border border-border bg-surface p-5 xl:sticky xl:top-6 xl:self-start">{selectedSession && selectedSeries ? <><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold text-muted-foreground">{trackLabel(selectedSeries.track)} · {selectedSession.week_label}</p><h2 className="mt-2 text-xl font-black">{selectedSession.title}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{selectedSession.summary}</p></div><IconButton label={`編輯 ${selectedSession.title}`} onClick={() => openEditSession(selectedSession)}><Edit3 size={17} /></IconButton></div><div className="mt-5 flex items-center justify-between border-y border-border py-3"><p className="font-bold">本堂教材與影片</p><button onClick={() => openNewResource(selectedSession)} className="inline-flex min-h-11 items-center gap-2 px-2 text-sm font-bold text-secondary"><Plus size={17} />新增內容</button></div><div className="mt-3 grid gap-3">{selectedSession.resources.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">這堂課還沒有教材或影片。</p> : selectedSession.resources.map((resource) => <article key={resource.id} className="border border-border bg-background p-4"><div className="flex items-start justify-between gap-3"><div className="flex gap-3">{resource.resource_type === "video" ? <Film className="mt-0.5 shrink-0 text-primary" size={19} /> : <BookOpen className="mt-0.5 shrink-0 text-secondary" size={19} />}<div><h3 className="font-bold">{resource.title}</h3><p className="mt-1 text-xs text-muted-foreground">{resourceTypeLabel(resource.resource_type)} · {visibilityLabel(resource.visibility)}</p></div></div><IconButton label={`編輯 ${resource.title}`} onClick={() => openEditResource(resource)}><Edit3 size={16} /></IconButton></div><p className="mt-3 text-sm leading-6 text-muted-foreground">{resource.description}</p>{(resource.youtube_url || resource.url) && <a href={resource.youtube_url ?? resource.url ?? "#"} target="_blank" rel="noreferrer" className="mt-3 inline-flex min-h-11 items-center gap-1 text-sm font-bold text-secondary">開啟內容<ExternalLink size={15} /></a>}</article>)}</div></> : <div className="grid min-h-64 place-items-center text-center text-sm text-muted-foreground">請從左側選擇一堂課管理教材與影片。</div>}</aside>
          </div>
        )}
      </div>
    </main>
...
function EditorShell({ title, onClose, children }: { title: string; onClose: () => void; children: ReactNode }) {
  return <section className="mt-7 border border-border bg-surface p-5 shadow-[4px_5px_0_var(--color-secondary)] md:p-7"><div className="mb-6 flex items-center justify-between"><div><p className="text-xs font-bold text-secondary">編輯區</p><h2 className="mt-1 text-xl font-black">{title}</h2></div><button type="button" onClick={onClose} aria-label="關閉編輯區" className="grid size-11 place-items-center"><X size={19} /></button></div>{children}</section>;
}
