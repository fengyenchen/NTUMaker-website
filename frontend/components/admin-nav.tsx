  ...
  return (
    <nav className="grid grid-cols-2 gap-1 sm:grid-cols-4 lg:grid-cols-1" aria-label="管理後台導覽">
      {navigation.map(([label, href, Icon]) => {
        ...
        return <Link key={href} href={href} aria-current={active ? "page" : undefined} className={`flex min-h-11 items-center gap-3 rounded-lg px-3 text-sm font-bold transition-colors hover:bg-background ${active ? "bg-background text-secondary" : "text-muted-foreground"}`}><Icon size={18} />{label}</Link>;
      })}
    </nav>
  );
}
