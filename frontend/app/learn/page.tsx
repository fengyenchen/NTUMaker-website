      ...
      <section className="px-5 py-16 md:px-8 md:py-24">
        <div className="mx-auto max-w-330">
          <div className="flex flex-col justify-between gap-5 md:flex-row md:items-end">
            <div>
              <p className="text-sm font-bold text-primary">社員學習區</p>
              <h1 className="mt-3 text-5xl font-black leading-[1.1] tracking-tight">
                {user.display_name
                  ? `${user.display_name}，繼續動手做。`
                  : "歡迎回來，繼續動手做。"}
              </h1>
            ...
            <div className="flex items-center gap-4 border border-border bg-surface px-5 py-4">
              <div>
                <p className="text-xs text-muted-foreground">社員資格</p>
          ...
          <div className="mt-12 grid gap-7 lg:grid-cols-[1.35fr_.65fr]">
            <section className="rounded-2xl bg-secondary p-8 text-on-secondary shadow-[6px_7px_0_var(--color-shadow-soft)] md:p-10">
              ...
              <h2 className="mt-4 text-4xl font-black">03 感測器與互動輸入</h2>
              <p className="mt-4 max-w-xl leading-7 text-white/75">
                認識類比與數位輸入，將感測數值整理成可以控制作品狀態的訊號。
              </p>
              <div className="mt-10 h-3 overflow-hidden rounded-full bg-black/20">
                <div className="h-full w-[38%] rounded-full bg-warning" />
              </div>
              ...
              <Link
                href="/resources"
                className="card-inline-link mt-8 inline-flex min-h-12 items-center gap-2 rounded-xl bg-background px-5 font-bold text-foreground"
              >
                進入本週課程{" "}
                <ArrowRight
            ...
            <section className="rounded-2xl border border-border bg-surface p-8">
              <CalendarClock className="text-primary" aria-hidden="true" />
              <h2 className="mt-8 text-2xl font-black">下一堂社課</h2>
              <p className="mt-2 text-muted-foreground">星期五 19:00</p>
              <p className="mt-6 text-xl font-bold">雷射切割與向量設計</p>
          ...
          <div className="mt-16 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
            <div>
              <p className="text-sm font-bold text-primary">已解鎖</p>
              <h2 className="mt-2 text-3xl font-black">社員教材與影片</h2>
            </div>
            <p className="text-sm text-muted-foreground">
              共 {resources.length} 項可用內容
            </p>
          </div>
          {resources.length ? (
            ...
            <div className="mt-6 border border-border bg-surface p-8 text-muted-foreground">
...
function ResourceCard({ resource }: { resource: MemberResource }) {
    ...
    <>
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="rounded-full bg-surface-raised px-3 py-1 text-xs font-bold">
          {resource.visibility === "member" ? "社員限定" : "公開"}
        </span>
      </div>
  ...
  return youtubeEmbedUrl ? (
    ...
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className="card-interactive border border-border bg-surface p-6"
    >
      {content}
    </a>
  ) : (
