...
from app.services.r2 import delete_image, read_image, upload_announcement_image, upload_image, upload_resource_file
...
@router.post("/uploads/announcement-images", summary="上傳公告圖片")
def upload_admin_announcement_image(file: UploadFile = File(...)) -> dict[str, str]:
    uploaded = upload_announcement_image(file)
    proxy_url = f"/api/v1/content/assets/{quote(uploaded['r2_object_key'], safe='/')}"
    return {
        "url": proxy_url,
        "public_url": uploaded["public_url"],
